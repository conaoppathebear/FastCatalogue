...
import { LandscaperA } from "@/samples/landscaper/LandscaperA";
import { LandscaperB } from "@/samples/landscaper/LandscaperB";
import { LandscaperC } from "@/samples/landscaper/LandscaperC";
import { PainterA } from "@/samples/painter/PainterA";
import { PainterB } from "@/samples/painter/PainterB";
import { PainterC } from "@/samples/painter/PainterC";
import { RooferA } from "@/samples/roofer/RooferA";
import { RooferB } from "@/samples/roofer/RooferB";
import { RooferC } from "@/samples/roofer/RooferC";
import { LocksmithA } from "@/samples/locksmith/LocksmithA";
import { LocksmithB } from "@/samples/locksmith/LocksmithB";
import { LocksmithC } from "@/samples/locksmith/LocksmithC";
import { CleanerA } from "@/samples/cleaner/CleanerA";
import { CleanerB } from "@/samples/cleaner/CleanerB";
import { CleanerC } from "@/samples/cleaner/CleanerC";
...
export default function Preview() {
  ...
  const renderTheme = () => {
    ...
    if (slug === 'landscaper') {
      if (themeId === 'clean') return <LandscaperA />;
      if (themeId === 'bold') return <LandscaperB />;
      if (themeId === 'luxury') return <LandscaperC />;
    }

    if (slug === 'painter') {
      if (themeId === 'clean') return <PainterA />;
      if (themeId === 'bold') return <PainterB />;
      if (themeId === 'luxury') return <PainterC />;
    }

    if (slug === 'roofer') {
      if (themeId === 'clean') return <RooferA />;
      if (themeId === 'bold') return <RooferB />;
      if (themeId === 'luxury') return <RooferC />;
    }

    if (slug === 'locksmith') {
      if (themeId === 'clean') return <LocksmithA />;
      if (themeId === 'bold') return <LocksmithB />;
      if (themeId === 'luxury') return <LocksmithC />;
    }

    if (slug === 'cleaner') {
      if (themeId === 'clean') return <CleanerA />;
      if (themeId === 'bold') return <CleanerB />;
      if (themeId === 'luxury') return <CleanerC />;
    }
